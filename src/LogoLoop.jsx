...
const LogoLoop = React.memo(({
  ...
  const renderLogoItem = useCallback(
    (item, key) => {
      ...
      const content = isNodeItem ? (
        <span className="logoloop-content" aria-hidden={!!item.href && !item.ariaLabel}>
          {item.node}
        </span>
      ) : (
        <img
  ...
  const logoLists = useMemo(
    () =>
      Array.from({ length: copyCount }, (_, copyIndex) => (
        <ul
          className={cx('logoloop-list', isVertical && 'logoloop-list-vertical')}
          key={`copy-${copyIndex}`}
          role="list"
  ...
  return (
    <div ref={containerRef} className={rootClasses} style={containerStyle} role="region" aria-label={ariaLabel}>
      ...
      <div
        className={cx('logoloop-track', isVertical ? 'logoloop-track-vertical' : 'logoloop-track-horizontal')}
        ref={trackRef}
